...
@MainActor
final class MotchillRemoteConfigTests: XCTestCase {
    ...
    func testClientFetchesRemoteConfigFromEndpoint() async throws {
        ...
        let client = MotchillRemoteConfigClient(session: session)
    ...
    func testAPIClientFailsImmediatelyWithoutRemoteConfig() async {
        ...
        let client = MotchillAPIClient(configuration: AppConfiguration())

        do {
            _ = try await client.fetchHomeSections()
            XCTFail("Expected remoteConfigUnavailable")
        } catch let error as MotchillAPIError {
            switch error {
            ...
            XCTFail("Expected MotchillAPIError, got \(error)")
        }
    }
}
